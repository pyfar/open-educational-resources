...
from BlockConvolver_DSP import BlockConvolver_DSP
from DSP import DSP
...
class Binaural_DSP(DSP):
    def __init__(self, config, num_src):
        ...
        if self.num_src:
            self.convolver_left = BlockConvolver_DSP(self.blockSize, self.numberOfInputs, self.hrirs.shape[2])
            self.convolver_right = BlockConvolver_DSP(self.blockSize, self.numberOfInputs, self.hrirs.shape[2])
            self.checkConfig()
