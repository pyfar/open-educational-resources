...
class VariableDelay_DSP(DSP):
    ...
    def process(self, inSig, prevBlock=np.zeros((1024,1))):
        ...
        for ch in range(self.numberOfInputs):
            ...
            valid_length = min(blockSize * maxSize, len(self.blockStack[:, ch]))
            
            # If we don't have enough data, use a simple delay
            if valid_length == 0:
                outSig[:, ch] = inSig[:, ch]
                continue
                
            x_points = np.arange(valid_length)
            y_points = self.blockStack[:valid_length, ch]
            
            # Create interpolation function with valid data
            interpFunc = interp1d(x_points, y_points, kind='linear', fill_value=0, bounds_error=False)
            
            # Calculate delayed indices with bounds checking
            delayedIndices = np.arange(blockSize) - delayCurve[:, ch] + blockSize * (maxSize - 1)
            
            # Clip indices to valid range
            delayedIndices = np.clip(delayedIndices, 0, valid_length - 1)
